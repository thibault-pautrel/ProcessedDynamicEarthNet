...
# Choose one pixel every SUBSAMPLE=8 pixels. 
# Original spatial resolution: 1024*1024. 
# With SUBSAMPLE=8, spatial resolution becomes 128*128.
...
def load_tif_as_tensor(filepath, subsample=1):
    ...
    with rasterio.open(filepath) as src:
        img = src.read()
        if subsample > 1:
            img = img[:, ::subsample, ::subsample]  # typically shape: (bands, H, W)
...
def get_label_folder_name(time_series_id, planet_id, label_base_path):
    """Return the label folder name that exists using either underscore or hyphen."""
    folder_us = f"{time_series_id}_{planet_id}"
    folder_hy = f"{time_series_id}-{planet_id}"
    if os.path.exists(os.path.join(label_base_path, folder_us)):
        return folder_us
    elif os.path.exists(os.path.join(label_base_path, folder_hy)):
        return folder_hy
    else:
        # Default to underscore if neither exists.
        return folder_us

def process_time_series(series_path, label_base_path, output_base, subsample=SUBSAMPLE):
    """
    ...
    # Use the helper function to support both underscore and hyphen naming
    label_folder_name = get_label_folder_name(time_series_id, planet_id, label_base_path)
    ...
    if os.path.exists(raster_label_base):
        raster_label_folders = os.listdir(raster_label_base)
        if raster_label_folders:
            raster_tile_folder = os.path.join(raster_label_base, raster_label_folders[0])
    ...
    for month, files in monthly_files.items():
        ...
        for dt, file in files_sorted:
            ...
            if os.path.exists(daily_out_file):
                ...
                continue
            try:
                tensor = load_tif_as_tensor(file, subsample=subsample)  # expected shape: (bands, H, W)
        ...
        if os.path.exists(raster_label_out_file):
            ...
            if raster_tile_folder:
                ...
                if candidates:
                    raster_tensor = load_tif_as_tensor(candidates[0], subsample=subsample)
...
def main():
    ...
    output_base = "/media/thibault/DynEarthNet/subsampled_data"
            ...
            for dirpath, dirnames, filenames in os.walk(planet_path):
                if os.path.basename(dirpath) == "PF-SR":
                    ...
                    if tile_has_nonempty_labels(dirpath, label_base):
                        ...
                        process_time_series(dirpath, label_base, output_base, subsample=SUBSAMPLE)
                    else:
